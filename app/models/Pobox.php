...
class Pobox extends \Crud {


    protected $primaryKey = 'id_pobox'; // !important

    protected $table = 'pobox';
    ...
    protected $crud = [
        ...
        // 
        "labels"    => ['id_pcustomer'=>'Customer','driverid'=>'Driver ID'],
        //
        ...
        "create"    => [],
        "edit"      => [],
        "index"     => ['id_pobox','id_customer','sender','pieces','info','status'],
        "show"      => [],
    ...
    public function pobox_items($id,$record)
    {
        $table     = "pobox_items";
        $arguments = [
            "index"        => ["id_pobox_items","item_description","carrier_name","Tracking","weight","unit","declared_value","location"]
        ];
        $where = ["where" => ["id_pobox"=>$id]];
        //id_awb, date, origin, destination , pieces, weight (+ unit)
    ...
    public function info($id,$record)
    {

        $records = DB::table('pobox')
                    ->join('pobox_items', 'pobox_items.id_pobox', '=', 'pobox.id_pobox')
                    ->where('pobox.id_awb',$id)
                    ->select('pobox.created_at','pobox_items.item_description', 'pobox_items.weight', 'pobox_items.location')
                    ->get();

        //id_awb, date, origin, destination , pieces, weight (+ unit)
        return \View::make("admin.pobox.tabs.info")
            ->with('records',$records);

            
// //        $table     = "awb";
// //        $arguments = [
// //            "index"        => ["date","origin","destination","pieces","weight","unit"]
// //        ];
// //        $where = ["where" => ["id_pcustomer"=>$id]];
// //        //id_awb, date, origin, destination , pieces, weight (+ unit)
// // 			## TODOMM : Ejemplo de que cargue el tab que estan en view.  Ya quedo
// //                  Reviar Vista, la idea es mostar algunos datos que se calculen aqui
// //                   por favor hasme un ejemplo con cargar un numero de AWB, cualqueir where, yo lo cambio
// 					$awb = 23;
// 					$record->id_awb = 23; 
    ...
    public function tracklog($id,$record)
    {
        $table     = "awb";
        ...
        $where = ["where" => ["id_awb"=>$record->id_awb]];
        //id_awb, date, origin, destination , pieces, weight (+ unit)
    ...
    {
        $class = new \PoboxInvoices();


        $records = $class->where("id_pcustomer",$id)
                     ->paginate(25);

        $path     = "pobox.tabs.invoices";
