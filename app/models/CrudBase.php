...
class CrudBase extends \BaseModel {
    ...
    // auto show, create and update

    public function _show(){
        return show($this);
    }

    public function _create($inputs = [],$validations = []){
    ...
    public function newQuery($isWith = false)
    {
                ...
                if($filter_queries and (Request::is(getenv('APP_ADMIN_PREFIX').'/*'))  or Request::is(getenv('APP_CUSTOMER_PREFIX').'/*'))
                {
                    ...
                    foreach ($filter_queries as $column => $table) {
                        if(is_numeric($column))
                        {
                            ...
                            $user = \Cache::remember($table."_id_users", 60, function() use ($users) {
                                return \DB::table($users->table)->where($users->getKeyName(),Auth::admin()->id())->first();
                            });

                            if(Schema::hasColumn($this->table, $table) )
                            {
    ...
    public $default_crud = [
        ...
        // ["first_name" => "text"] //"remotemultiple","multiple","radiogroup","radios","editor","toggle","html","text", "hidden", "digit", "textarea", "password", "email","datetime","date","time","select","autocomplete","money","currency","file","document","audio","video","zip"
        //
        "inputs"    => [],
    ...
    public $inputFiles   = ["file"=>"","image"=>"image","gallery"=>"image","images"=>"image","files"=>"","video"=>"video","audio"=>"audio","zip"=>"zip","document"=>"document"];
    ...
    public function customTab($id,$class,$path,$records){


        $path      = $this->getPathView($path);
    ...
    public function tab($table,$where = null,$arguments=[],$view= null)
    {
        ...
        }elseif(is_array($where))
        {
            ...
            foreach ($where as $function => $function_arguments) {
                ...
                if($function == "where")
                {
                    if(array_key_exists($current->getKeyName(), $function_arguments))
                        $key_value = $function_arguments[$current->getKeyName()];
        ...
        $current  = $this->toTable($current_class_name).".tabs.".$viewName;

        $path     = $this->getPathView($current,$default_view);
    ...
    public function getColumns($id = "")
    {
        ...
        foreach ($columns as $column) {
            ...
            if($type == "enum")
            {
                ...
                {
                    $enum_data = $enums[$column->getName()];
        ...
        foreach ($columns as $column) {
            ...
            else
            {
                ...
                if($column->name != $this->getKeyName())
                {
                    ...
                    if( in_array($column->type, $numeric))
                        $rule[] = "numeric";
