...
class Payments extends \Crud {


    protected $primaryKey = 'id_payments'; // !important
    ...
    protected $crud = [
        ...
        // 
        "labels"    => ['paydate'=>'Date','id_pcustomer'=>'Customer'],
        //
        ...
        //
        "create"    => ['id_pcustomer','paydate','description','paymethod','amount','ptype'],
        "edit"      => [],
        "index"     => ['id_payments','id_pcustomer','paydate','amount'],
        "show"      => [],
    ...
    public function lines($id,$record)
    {

        if ($record->ptype == 'invoice'){
        	$records = DB::table('payments_inf')
                    ->join('invoice', 'invoice.id_invoice', '=', 'payments_inf.ref_num')
                    ->where('payments_inf.id_payments',$id)
                    ->select('invoice.id_invoice','invoice.invoicenum', 'invoice.date', 'invoice.total','payments_inf.amount')
                    ->get();

			return \View::make("admin.payments.tabs.invoices")
            ->with('records',$records);
            
		}else{
        	$records = DB::table('payments_inf')
                    ->join('awb', 'invoice.id_awb', '=', 'payments_inf.ref_num')
                    ->where('payments_inf.id_payments',$id)
                    ->select('invoice.id_invoice','invoice.invoicenum', 'invoice.date', 'invoice.total','payments_inf.amount')
                    ->get();

			return \View::make("admin.payments.tabs.awbs")
            ->with('records',$records);
