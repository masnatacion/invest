...
{{ Form::open( array('url' => array(getenv('APP_ADMIN_PREFIX').'/'.$model.'/'), 'method' => 'GET','class'  => $class." margin-top-lg",'role' =>'search')) }}
...
@foreach ($advance_search as $column)
     ...
     <div class="form-group @if ($errors->has($column->name)) has-error @endif">
        ...
        <div class="col-md-10">
         ...
         @elseif ($column->input == "remotemultiple")

            {{ Form::remotemultiple($column->name,Input::get($column->name),['table'=>$model,'class' => 'form-control','placeholder'=>$column->label] ); }}

         @elseif ($column->input == "multiple")

            {{ Form::multiple($column->name,Input::get($column->name),['class' => 'form-control','placeholder'=>$column->label],$column->data); }}
        ...
        @elseif ($column->input == "audio" or  $column->input == "image" or  $column->input == "video" or  $column->input == "document" or  $column->input == "zip" or  $column->input == "file")
            ...
            {{ Form::filepicker($column->name,Input::get($column->name)) }}
         ...
         @elseif  ($column->input == "toggle") 
            ...
            {{ Form::toggle($column->name, Input::get($column->name),[],$column->data); }}
            
         @elseif  ($column->input == "html" || $column->input == "editor") 

            {{ Form::editor($column->name, Input::get($column->name),[]); }}
