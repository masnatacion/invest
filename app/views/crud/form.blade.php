     ...
     <div class="form-group @if ($errors->has($column->name)) has-error @endif">
        ...
        @endif
         ...
         @elseif ($column->input == "remotemultiple")
            ...
            {{ Form::remotemultiple($column->name,$record->{$column->name},['table'=>$model,'class' => 'form-control','placeholder'=>$column->label] ); }}

         @elseif ($column->input == "multiple")

            {{ Form::multiple($column->name,$record->{$column->name},['class' => 'form-control','placeholder'=>$column->label],$column->data); }}
