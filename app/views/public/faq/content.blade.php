<div class="faq">
	<div class="row ">
		<div class="col-md-8">
			<h2>{{ $record->txt_title }}</h2>

			
			<div class="box">
				<div class="row margin-bottom-lg">
					<div class="col-md-3">
						<img src="data:image/svg+xml;base64," alt="..." class="img-thumbnail">	
					</div>
					<div class="col-md-9">

						<h4>{{ $record->txt_subtitle1 }}</h4>
						<ul>
							{{ $record->html_body1 }}
						</ul>
						<br>

						<h4>{{ $record->txt_subtitle2 }}</h4>
						<ul>
							{{ $record->html_body2 }}
						</ul>
						<br>
						
						<h4>{{ $record->txt_subtitle3 }}</h4>
						<ul>
							{{ $record->html_body3 }}
						</ul>
						<br>

						<h4>{{ $record->txt_subtitle4 }}</h4>
						<ul>
							{{ $record->html_body4 }}
						</ul>
						<br>


						<h4>{{ $record2->txt_subtitle1 }}</h4>
						<ul>
							{{ $record2->html_body1 }}
						</ul>
						<br>

						<h4>{{ $record2->txt_subtitle2 }}</h4>
						<ul>
							{{ $record2->html_body2 }}
						</ul>
						<br>
						
						<h4>{{ $record2->txt_subtitle3 }}</h4>
						<ul>
							{{ $record2->html_body3 }}
						</ul>
						<br>

						<h4>{{ $record2->txt_subtitle4 }}</h4>
						<ul>
							{{ $record2->html_body4 }}
						</ul>
						<br>


						<h4>{{ $record3->txt_subtitle1 }}</h4>
						<ul>
							{{ $record3->html_body1 }}
						</ul>
						<br>

						<h4>{{ $record3->txt_subtitle2 }}</h4>
						<ul>
							{{ $record3->html_body2 }}
						</ul>
						<br>
						
						<h4>{{ $record3->txt_subtitle3 }}</h4>
						<ul>
							{{ $record3->html_body3 }}
						</ul>
						<br>

						<h4>{{ $record3->txt_subtitle4 }}</h4>
						<ul>
							{{ $record3->html_body4 }}
						</ul>
						<br>




					</div>
				</div>
			</div>



				
		</div>
	</div>
</div>
